...
character = P parseChar
  where parseChar Nil = UnexpectedEof
        parseChar (hd :. tl) = Result tl hd
...
P pa ||| P pb = P $ \i ->
  let resultA = pa i
  in if isErrorResult resultA then
       pb i
     else
       resultA
