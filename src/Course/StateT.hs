...
runState' ::
  ...
  -> s
  -> (a, s)
runState' (StateT a) = runExactlyOne . a
...
distinctF ::
  ...
  -> Optional (List a)
distinctF xs =
  let pred a = StateT $ \s -> if a > 100 then Empty else Full (S.notMember a s, S.insert a s)
      result = filtering pred xs
  in evalT result S.empty
...
distinctG xs =
  let result = filtering (StateT . ((OptionalT <$>) <$> distCondition)) xs
  in runOptionalT $ evalT result S.empty
...
distCondition a s
  | a > 100 = log1 (listh "aborting > 100: " ++ show' a) Empty
  | even a = log1 (listh "even number: " ++ show' a) v
  | otherwise = log0 v
  where v = Full (S.notMember a s, S.insert a s)
